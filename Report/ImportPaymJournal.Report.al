report 50012 "Import Payment Journal"
{
    ...
    begin
        ...
        GenJournalLine."Account No." := GetValueAtCell(RowNo, 7);
        Evaluate(GenJournalLine."Posting Date", GetValueAtCell(RowNo, 4));
        GenJournalLine."Document Type" := GenJournalLine."Document Type"::Payment;
        GenJournalLine.Validate("Document No.", GetValueAtCell(RowNo, 8));
        GenJournalLine.Validate(Description, GetValueAtCell(RowNo, 9));
        GenJournalLine.Validate("Bal. Account Type", GenJournalLine."Bal. Account Type"::"Bank Account");
        GenJournalLine.Validate("Bal. Account No.", GetValueAtCell(RowNo, 11));
        Evaluate(ConvInt, GetValueAtCell(RowNo, 12));
        GenJournalLine.Validate(Amount, ConvInt);
        GenJournalLine.Validate("Amount (LCY)", ConvInt);
        GenJournalLine.Validate("Shortcut Dimension 1 Code", GetValueAtCell(RowNo, 13));
        GenJournalLine.Validate("Shortcut Dimension 2 Code", GetValueAtCell(RowNo, 14));
        If GetValueAtCell(RowNo, 15) IN ['Pagamento eletrónico', 'Electronic Payment'] then begin
            GenJournalLine.Validate("Bank Payment Type", 3);
        End;
        ...
        If GetValueAtCell(RowNo, 15) IN ['Cheque Manual', 'Manual Check'] then begin
            GenJournalLine.Validate("Bank Payment Type", 2);
        End;
        If GetValueAtCell(RowNo, 15) IN ['Documento eletrónico-IAT', 'Electronic Payment-IAT'] then begin
            GenJournalLine.Validate("Bank Payment Type", 4);
        End;
        GenJournalLine.Validate("Recipient Bank Account", GetValueAtCell(RowNo, 16));
        GenJournalLine.Validate("PTSS Bal: cash-flow code", GetValueAtCell(RowNo, 17));
