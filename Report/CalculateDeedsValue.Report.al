report 57018 "Calculate Deeds Value"
{
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            DataItemTableView = SORTING("No.") WHERE(Type = CONST(Inventory), "Currency Code" = filter(<> ''));
            ...
            begin
                ...
                CalcFields("Deed Balance", Inventory);
                ...
                if ItemLedgEntry.Find('-') then
                    repeat
                        if IncludeEntryInCalc(ItemLedgEntry, PostingDate, IncludeExpectedCost) then begin
                            ...
                            RemCost := Round("Deed Balance" * (RemQty * 1 / Inventory));
    ...
    var
        ...
        Text012: Label 'The item %1 dont have inventory.';
