report 57012 "Sales Invoice"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Report/RDL/SalesInvoice.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(zDateHour; STRSUBSTNO(zText003, FORMAT("Shipment Date", 0, '<Year4>/<Month,2>/<Day,2>'), FORMAT("PTSS Shipment Start Time", 0, 0), ''))
            {
            }
            column(zVATCashAccDesc; g_VATCashAccDesc)
            {
            }
            column(zManualInvoice; "PTSS Manual Invoice")
            {
            }
            column(zManualInvoiceNo; STRSUBSTNO('%1/%2', "PTSS Manual Inv. Series", "PTSS Manual Invoice No."))
            {
            }
            column(zManualInvoiceText; g_ManualInvoiceText)
            {
            }
            column(int1; int[1])
            {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(DocumentCaptionCopyText; STRSUBSTNO(Text200, "Sales Invoice Header"."No."))
                    {
                    }
                    ...
                    column(PostingDate_SalesInvHdr; FORMAT("Sales Invoice Header"."Posting Date", 0, '<Year4>-<Month,2>-<Day,2>'))
                    {
                    }
                    ...
                    column(DueDate_SalesInvHdr; FORMAT("Sales Invoice Header"."Due Date", 0, '<Year4>-<Month,2>-<Day,2>'))
                    {
                    }
                    ...
                    column(DocDate_SalesInvHdr; FORMAT("Sales Invoice Header"."Document Date", 0, '<Year4>-<Month,2>-<Day,2>'))
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            IF "Sales Invoice Header"."Prices Including VAT" THEN
                                "Line Amount" := Amount;
                            IF g_VATPostingSetup.GET("Sales Invoice Line"."VAT Bus. Posting Group", "Sales Invoice Line"."VAT Prod. Posting Group") THEN BEGIN
                                VATAmountLine.INIT;
                            ...
                            IF ("Sales Invoice Line".Type = "Sales Line Type"::Item) AND ("Sales Invoice Line".Quantity <> 0) THEN BEGIN
            ...
            begin
                ...
                g_ManualInvoiceText := '';
                IF "PTSS Manual Invoice" THEN
                    g_ManualInvoiceText := zText52076163;
                ...
                PaymentMethods.GET("Sales Invoice Header"."Payment Method Code");
                CustomerBankAccount.Reset();
                CustomerBankAccount.SETRANGE("Customer No.", "Sales Invoice Header"."Bill-to Customer No.");
                ...
                IF (PaymentMethods."Bal. Account Type" = "Payment Balance Account Type"::"Bank Account") AND (PaymentMethods."Bal. Account No." <> '') THEN
                    texto2 := "Sales Invoice Header"."Payment Method Code" + ' ' + CustomerBankAccount.IBAN
                ELSE
                    texto2 := "Sales Invoice Header"."Payment Method Code";
                PaymentTerms.GET("Sales Invoice Header"."Payment Terms Code");
                IF FORMAT(PaymentTerms."Due Date Calculation") = '0D' THEN BEGIN
                    texto[3] := text202;
                    texto[4] := STRSUBSTNO(text210, CompanyInfo.IBAN, CompanyInfo."SWIFT Code");
                END;

                QRCodeBase64 := PTSSBarCode.GetQRCodeStringBase64(DocumentType::"Sales Invoice", "Sales Invoice Header"."No.");
                ATCud := ATCudCaption + g_CreateSignature.GetATCud(DocumentType::"Sales Invoice", "Sales Invoice Header"."No.");
            end;
        }
    }
    ...
    var
        ...
        Text200: Label 'Receipt - Invoice %1';
        ...
        text210: Label 'IBAN: %1 SWIFT Code: %2';
    ...
    local procedure FillAll(SalesInvoiceHeader: Record "Sales Invoice Header")
    var
        SalesInvoiceLine: Record "Sales Invoice Line";
    ...
    begin
        ...
        IF SalesInvoiceLine.FINDSET THEN
            REPEAT
                IF (SalesInvoiceLine.Type = "Sales Line Type"::Item) AND (SalesInvoiceLine.Quantity <> 0) THEN BEGIN
                ...
                IF IVAText = '' THEN BEGIN
                    IF (SalesInvoiceLine."Amount Including VAT" - SalesInvoiceLine.Amount) <> 0 THEN
                        IVAText := VATAmountLine.VATAmountText
                    ELSE
                        IVAText := '';
                END;

                TotalAmountSemIva += SalesInvoiceLine.Amount;
                TotalAmountComIva += SalesInvoiceLine."Amount Including VAT";
            UNTIL SalesInvoiceLine.NEXT = 0;

        TotalIVA := TotalAmountComIva - TotalAmountSemIva;

        IF "Sales Invoice Header"."PTSS Withholding Invoice" THEN
