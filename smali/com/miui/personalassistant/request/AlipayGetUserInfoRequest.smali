.class public Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;
.super Lcom/miui/personalassistant/request/core/HttpBizRequest;
.source "AlipayGetUserInfoRequest.java"


# instance fields
.field private authToken:Ljava/lang/String;


# direct methods
.method public constructor <init>(Landroid/content/Context;Ljava/lang/String;Ljava/lang/String;)V
    .locals 5

    const/4 v4, 0x0

    invoke-direct {p0, p1}, Lcom/miui/personalassistant/request/core/HttpBizRequest;-><init>(Landroid/content/Context;)V

    iput-object p2, p0, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->authToken:Ljava/lang/String;

    invoke-virtual {p0, v4}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->setSignUrlAndParams(Z)V

    invoke-virtual {p0}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->getUrl()Ljava/lang/String;

    move-result-object v0

    invoke-virtual {p0, v0}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->setUrl(Ljava/lang/String;)V

    const-string/jumbo v0, "app_id"

    const-string/jumbo v1, "2017050507133157"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "method"

    const-string/jumbo v1, "alipay.user.info.share"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "charset"

    const-string/jumbo v1, "utf-8"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "sign_type"

    const-string/jumbo v1, "RSA2"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "timestamp"

    invoke-static {}, Ljava/lang/System;->currentTimeMillis()J

    move-result-wide v2

    invoke-static {v2, v3}, Lcom/miui/personalassistant/util/TimeUtil;->getFormatTimeString(J)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "version"

    const-string/jumbo v1, "1.0"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "grant_type"

    const-string/jumbo v1, "authorization_code"

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "auth_token"

    invoke-virtual {p0, v0, p2}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "code"

    invoke-virtual {p0, v0, p3}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    const-string/jumbo v0, "sign"

    iget-object v1, p0, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->mParamMap:Ljava/util/HashMap;

    const-string/jumbo v2, ""

    const/4 v3, 0x1

    invoke-static {v1, v2, v3, v4}, Lcom/miui/personalassistant/util/AlipayOrderInfoUtil2_0;->getSign(Ljava/util/Map;Ljava/lang/String;ZZ)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, v0, v1}, Lcom/miui/personalassistant/request/AlipayGetUserInfoRequest;->addParam(Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method


# virtual methods
.method public getUrl()Ljava/lang/String;
    .locals 1

    const-string/jumbo v0, "https://openapi.alipay.com/gateway.do"

    return-object v0
.end method
