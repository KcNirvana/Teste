page 57001 "Budget Class 0 Matrix"
{
    ...
    begin
        case DimType of
            ...
            DimType::"Global Dimension 1":
                begin
                    if BudgetDimType <> BudgetDimType::" " then
                        SetBudgetDimensionValueFilters(DimVal, GlobalDim1Filter, GLSetup."Global Dimension 1 Code", DimCodeBuf)
                    else
    ...
    local procedure SetBudgetDimensionValueFilters(var DimensionValue: Record "Dimension Value"; CodeFilter: Code[250]; SetupDimensionCode: Code[20]; DimensionCodeBuffer: Record "Dimension Code Buffer")
    begin
        if CodeFilter <> '' then
            DimensionValue.SetFilter(Code, CodeFilter);
        DimensionValue."Dimension Code" := SetupDimensionCode;
        DimensionValue.SetRange("Dimension Code", DimensionValue."Dimension Code");
        DimensionValue.SetRange("Budget Dim Type", BudgetDimType);
        DimensionValue.Code := DimensionCodeBuffer.Code;
    end;
    ...
    begin
        with TheDimCodeBuf do begin
            ...
            "Show in Bold" := TheDimVal.Blocked;
            "Budget Dim Type" := TheDimVal."Budget Dim Type";
        end;
    end;

    local procedure LookUpCode(DimType: Enum "G/L Budget Matrix Dimensions"; DimCode: Text[30];
                                            "Code": Text[30])
    var
        GLAcc: Record "G/L Account";
        BusUnit: Record "Business Unit";
    ...
    begin
        with TheGLAccBudgetBuf do begin
            ...
            SetRange("G/L Account Filter", EndowmentAccount);
            if BusUnitFilter <> '' then
            ...
            if BudgetMovType <> BudgetMovType::" " then
                SetRange("Budget Mov Type Filter", BudgetMovType)
            else
                SetRange("Budget Mov Type Filter");
        end;
    end;
    ...
    begin
        ...
        with TheGLAccBudgetBuf do begin
            ...
            if BudgetDimType <> BudgetDimType::" " then
                SetRange("Budget Dim Type", BudgetDimType)
            else
                SetRange("Budget Dim Type");
        end;
    end;
    ...
    begin
        ...
        GLAccBudgetBuf."G/L Account Filter" := EndowmentAccount;
        ...
        If GLAccBudgetBuf.GetFilter("Budget Mov Type Filter") <> '' then
            GLAccBudgetBuf.CopyFilter("Budget Mov Type Filter", GLBudgetEntry."Budget Mov Type");
        ...
        PAGE.Run(Page::"Budget Class 0 Entries", GLBudgetEntry);
    end;
    ...
    begin
        GLAccBudgetBuf.CalcFields("New Budgeted Amount");
        exit(GLAccBudgetBuf."New Budgeted Amount");
    end;
    ...
    begin
        ...
        InternalDateFilter := '';
        BudgetMovType := NewBudgetMovType;
        BudgetDimType := NewBudgetDimType;
        EndowmentAccount := NewEndowmentAccount;
    end;
    ...
    begin
        GLAccBudgetBuf.CalcFields("New Budgeted Amount");
        GLAccBudgetBuf.Validate("New Budgeted Amount", NewAmount);
    end;
